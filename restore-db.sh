...
RESTORE_OUTPUT=$(/opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P "$SA_PASSWORD" -Q "RESTORE DATABASE AdventureWorksDW FROM DISK = '/opt/mssql/backup/AdventureWorksDW.bak' WITH MOVE 'AdventureWorksDW2017' TO '/var/opt/mssql/data/AdventureWorksDW.mdf', MOVE 'AdventureWorksDW2017_log' TO '/var/opt/mssql/data/AdventureWorksDW_log.ldf', RECOVERY, REPLACE" 2>&1)
RESTORE_STATUS=$?
